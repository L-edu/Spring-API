...
const AddClient = () => {
  const [newClient, setNewClient] = useState({ name: "", email: "" , cpf: "", renda: ""});
  const router = useRouter();
 
  const handleInputChange = (e) => {
    setNewClient({ ...newClient, [e.target.name]: e.target.value }); //com spread
    //setNewClient({ name, email, [e.target.name]: e.target.value }); sem spread
  };
